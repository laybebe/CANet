...
class CFEANet(nn.Module):
    def __init__(self, model,n_classes):
        ...
        self.emau = CAE(512, 10*self.n_classes, settings.STAGE_NUM)
        self.gap = nn.AdaptiveAvgPool2d((1, 1))
        self.fc_base = nn.Linear(10240, self.n_classes)
    ...
    def forward(self, img):
        ...
        class_att=att_map.view(b,self.n_classes,-1,h,w).mean(dim=2)
        ...
        for i in range(self.n_classes):
            if i==0:
                ...
                x_temp=torch.matmul(idn,class_att[:,i,:,:].view(b,-1,h,w))
                x1=torch.cat((x1,x_temp),dim=1)
        # x1= x1.view(b,self.n_classes,-1,h,w).mean(dim=2) 
        x=x1
        x = self.gap(x)
        x = x.view(x.size(0), -1)
        pred = self.fc_base(x)

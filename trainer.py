...
from util import AverageMeter, AveragePrecisionMeter,visuaization
...
class Trainer(object):
    ...
    def on_end_epoch(self, is_train=False):
        ...
        if is_train:
            ...
            OP_k, OR_k, OF1_k, CP_k, CR_k, CF1_k = self.meters['ap_meter'].overall_topk(
                3)
    ...
    def on_forward(self, inputs, targets, is_train):
        if self.args.data == "CUB200":
            inputs = Variable(inputs).float()
            targets=Variable(torch.max(targets, dim=1)[1]).long()
        else:
            inputs = Variable(inputs).float()
        ...
        if not is_train:
            ...
            outputs1 = self.model(inputs)
        outputs = outputs1  # (outputs1 + outputs2) / 2
        # loss=self.criterion(outputs1,targets)
        loss_cls = self.criterion(outputs1, targets)
        # loss_enc = self.criterion(outputs2, targets)
        loss = loss_cls.mean()#+0*loss_enc.mean()
        ...
        if is_train:
            ...
            torch.nn.utils.clip_grad_norm_(
                self.model.parameters(), max_norm=self.args.max_clip_grad_norm)
            self.optimizer.step()
    ...
    def adjust_learning_rate(self):
        ...
        lr_list = []
        decay = 0.1 if sum(self.epoch == np.array(
            self.args.epoch_step)) > 0 else 1.0
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.end_epoch):
            ...
            if self.args.data == "VOC2012":
                score = self.run_iteration_voc2012(
                    self.val_loader, epoch, is_train=False)
            if self.args.data == "CUB200":
                score = self.run_iteration_CUB(
    ...
    def run_iteration(self, data_loader, is_train=True):
        ...
        for i, data in enumerate(data_loader):
            ...
            att_amp=self.model.module.att_map
            # visuaization(att_amp,raw_img, image_name)
            # measure elapsed time
            batch_time = time.time() - st_time
            self.meters['batch_time'].update(batch_time)

            self.meters['ap_meter'].add(
                outputs.data, labels.data, data['name'])
            st_time = time.time()
            ...
            if is_train and i % self.args.display_interval == 0:
                print('{}, {} Epoch, {} Iter, Loss: {:.4f}, Data time: {:.4f}, Batch time: {:.4f}'.format(
                    ...
                    self.meters['loss'].value(
                    ), self.meters['data_time'].value(),
                    self.meters['batch_time'].value()))

        return self.on_end_epoch(is_train=is_train)

    def run_iteration_voc2012(self, data_loader, epoch, is_train=True):
        self.on_start_epoch()
        save_txt = self.args.save_dir + \
            "/epoch{}_result/VOC2012/Main".format(epoch)
        ...
        for i, data in enumerate(data_loader):
            ...
            outputs = self.on_forward(inputs, targets, is_train=is_train)
            score = self.sigmoid(outputs)

            for i, cls_name in enumerate(object_categories):
                with open(save_txt+"/comp1_cls_test_"+cls_name+".txt", "a+") as f:
                    for j, file_name in enumerate(filenames):
                        f.write(file_name+" "+f'{score[j][i]:.6f}'+"\n")

        return 0
    
    def run_iteration_CUB(self, data_loader, is_train=True):
        self.on_start_epoch()
        true_num = 0
        if not is_train:
            data_loader = tqdm(data_loader, desc='Validate')
        ...
        for i, data in enumerate(data_loader):
            ...
            predict_index = list(torch.max(score, dim=1)[1].cpu().numpy())
            label_index = list(torch.max(targets, dim=1)[1].cpu().numpy())
            intersection = [x for x in predict_index if x in label_index]
            true_num += len(intersection)
        return true_num
    ...
    def validate_2012(self):
        self.initialization(is_train=False)

        map = self.run_iteration_voc2012(self.val_loader, is_train=False)
        return map

    def validate_CUB200(self):
        self.initialization(is_train=False)

        true_num = self.run_iteration_CUB(self.val_loader, is_train=False)
        acc=true_num/5794
        return acc
        ...
        with open(res_path, 'w') as fid:
            for i in range(self.meters['ap_meter'].scores.shape[0]):
                fid.write('{},{},{}\n'.format(self.meters['ap_meter'].filenames[i],
                                              ','.join(
                                                  map(str, self.meters['ap_meter'].scores[i].numpy())),
